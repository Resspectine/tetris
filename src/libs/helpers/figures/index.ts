...
import { COLOR_BY_FIGURE_TYPE, INITIAL_COORDINATES_BY_FIGURE_TYPE } from './constants';
...
export type FigureType = 'I' | 'L' | 'Z' | 'RevertL' | 'RevertZ' | 'Square' | 'T';

export const createFigure = (
  type: FigureType,
  { offsetX, offsetY, initialCoordinate }: { offsetX?: number; offsetY?: number; initialCoordinate?: Coordinates[] }
): Figure => ({
  color: COLOR_BY_FIGURE_TYPE[type],
  coordinates:
    initialCoordinate ??
    INITIAL_COORDINATES_BY_FIGURE_TYPE[type].map(([x, y]) => [(offsetX || 0) + x, (offsetY || 0) + y]),
  coordinatesInNullishPoint: initialCoordinate ?? INITIAL_COORDINATES_BY_FIGURE_TYPE[type],
  type,
});
...
export const updateCoordinatesInNullishPoint = (
  ...
  figure: Figure
): Figure => ({ ...figure, coordinatesInNullishPoint: newCoordinatesInNullishPoint });
...
export const isFiguresCollided = (staticFigures: Figure[], currentFigure: Figure) => {
  return staticFigures.some(figure =>
    figure.coordinates.some(coords =>
      currentFigure.coordinates.some(selfCoords => coords[0] === selfCoords[0] && coords[1] === selfCoords[1])
    )
  );
};
