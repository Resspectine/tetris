...
import { createFigure, Figure, isFiguresCollided, updateCoordinatesInNullishPoint } from '../figures';
...
export const rotate = (figure: Figure, staticFigures: Figure[], isClockwise?: boolean): Figure => {
  const diff = new Matrix(figure.coordinates).add(new Matrix(figure.coordinatesInNullishPoint).neg());
  ...
  const newCoordinatesInNullishPoint = new Matrix(figure.coordinatesInNullishPoint).mmul(rotation).to2DArray();

  const figureWithNewCoordinatesInNullishPoint = updateCoordinatesInNullishPoint(newCoordinatesInNullishPoint, figure);
...
const coordinatesMapperByMoveType: Record<Moves, (figure: Coordinates) => Coordinates> = {
...
export const move = (figure: Figure, staticFigures: Figure[], moveTo: Moves, isEndCallback?: () => void): Figure => {
